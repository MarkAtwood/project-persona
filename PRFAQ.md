...
The SSH bouncer needs to validate identity credentials. Without personad, the bouncer would need to implement its own identity federation, presence attestation, and credential issuance — duplicating everything personad does. Building personad first means the SSH bouncer (and every other enforcement point) calls `FetchJWTSVID` on the persona socket and gets a signed credential. The bouncer validates the credential against the SPIFFE trust bundle. The alternative — each enforcement point implementing its own identity stack — is the fragmentation personad exists to eliminate.
