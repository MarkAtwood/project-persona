...
personad introduces three capabilities absent from every existing identity tool: per-audience pseudonymity, hardware presence attestation, and multi-source federation. Each consumer application receives a stable but opaque pseudonym derived from the (app-identity, root-identity) pair — the Apple Sign-In-with-Apple model lifted into SPIFFE ID paths. Consumers cannot correlate identities across applications without explicit user consent. Hardware presence is attested via FIDO2 touch, Windows Hello, TouchID, or PIV PIN, with timestamped claims that decay on a fixed TTL. Identity sources from different trust domains — a work IdP, a personal DID, a Tailscale tailnet — coexist without conflation.
...
### How does per-audience pseudonymity work?

By default, each consumer application receives a stable opaque pseudonym, not the user's root identity. The pseudonym is derived deterministically via HKDF-SHA256 from the (root identity key, trust domain, consumer app identity) triple. App A and App B each get a different identifier for the same user. Neither can derive the other's identifier. Cross-consumer linkage requires explicit user consent via `persona enroll`. This is the Apple Sign-In-with-Apple model applied at the OS level, not the browser level.
