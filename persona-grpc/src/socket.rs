...
//   (`\\.\pipe\persona-workload-{sid}`), which this function cannot express |
...
fn socket_under(runtime_dir: Option<PathBuf>, uid: u32) -> PathBuf {
    match runtime_dir {
        Some(dir) => dir.join("persona").join("workload.sock"),
        // The uid is in the directory name because /tmp is shared and world-writable.
        None => PathBuf::from(format!("/tmp/persona-{uid}")).join("workload.sock"),
    }
}
...
/// and taking `Some("")` here would yield the relative path `persona/workload.sock`,
...
mod tests {
    ...
    fn runtime_dir_gives_the_documented_systemd_path() {
        assert_eq!(
            ...
            PathBuf::from("/run/user/1000/persona/workload.sock")
        );
    }
    ...
    fn runtime_dir_gives_the_documented_macos_path() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/var/folders/1s/8j5xq0000gn/T/persona/workload.sock")
        );
    }
    ...
    fn fallback_gives_the_documented_non_systemd_path() {
        assert_eq!(
            ...
            PathBuf::from("/tmp/persona-1000/workload.sock")
        );
    }
