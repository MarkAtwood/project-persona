...
use hire_attestors::{probe_sources, Claim};
...
async fn the_os_is_an_identity_source_with_no_agent_no_token_and_no_cloud_account() {
    ...
    let empty_home = std::env::temp_dir().join(format!("hire-unix-bare-{}", std::process::id()));
    ...
    assert_eq!(
        ...
        "position is load-bearing and invisible in a per-file diff: \
         hire-grpc keeps the first claim seen at the winning tier, so an \
