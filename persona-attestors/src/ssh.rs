...
impl Attestor for SshAgentAttestor {
    ...
    async fn prove(
        ...
        if algorithm != SSH_ED25519 {
            ...
            //   candidate — a separate decision, it changes `persona enumerate`.
            let algorithm = String::from_utf8_lossy(algorithm).into_owned();
            tracing::info!(
                event = "ssh_key_unsupported",
                key = %candidate.path,
                algorithm = %algorithm,
                "persona proves ssh-ed25519 agent keys only"
            );
            return Err(AttestorError::ChallengeFailed(format!(
                "ssh-agent key {} is {algorithm}; persona proves ssh-ed25519 keys only",
                candidate.path
            )));
        }
