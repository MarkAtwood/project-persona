...
use persona_attestors::{Attestor, Claim, SshAgentAttestor};
...
async fn a_real_agent_key_proves_possession_and_claims_no_presence() {
    ...
    let dir = std::env::temp_dir().join(format!("persona-ssh-live-{}", std::process::id()));
    ...
    let candidate = candidates
        .iter()
        .find(|c| c.display_name == "persona-live-test")
