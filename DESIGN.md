...
This is a deliberate strategic choice. By presenting the standard SPIFFE Workload API -- a CNCF standard with gRPC proto definitions, client libraries in Go/Java/Python/Rust, and production deployment in every major service mesh -- `personad` inherits the entire SPIFFE ecosystem. Any SPIFFE-aware consumer works unmodified. Any platform that wants a different answer can implement the same API themselves.
...
**CLI:** `persona`
**Socket (Linux):** `$XDG_RUNTIME_DIR/persona/workload.sock`, normally `/run/user/{uid}/persona/workload.sock` (gRPC, SPIFFE Workload API)
...
Register `personad` as a FedCM identity provider. The browser handles the trust UI; web pages call the FedCM API without a custom extension. Timeline: months to years, depending on FedCM standardization velocity.
...
|---|---|---|
| Human identity + presence | `personad` | **this project** |
...
Covered in detail in the Delegated Access section below. The gateway speaks real IMAP and SMTP, validates `personad`-issued delegation grants (grant + client key + scope + TTL), and enforces send-as identity on SMTP.
...
1. **`personad`** -- 5-7 weeks -- foundation; nothing else works without it
